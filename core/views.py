...
def index(request):
    ...
    for users in user_following:
        feed_lists = Post.objects.filter(user=users.user)
...
def search(request):
    ...
    if request.method == 'POST':
        ...
        for users in username_obj:
            profile_list = Profile.objects.filter(id_user=users.id)
            user_profile_list.append(profile_list)
...
def follow(request):
    if request.method == 'POST':
        ...
        isExits = FollowersCount.objects.filter(
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                # Log user in and redirect to profile seeting page
